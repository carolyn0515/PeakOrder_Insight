...
test-python:
	...
	$(PYTHON) -c 'from pathlib import Path; paths=[Path("dags/peakorder_paimon_pipeline.py"), Path("src/quality/validate_order_events.py"), Path("src/paimon/bootstrap_tables.py"), Path("src/paimon/load_order_events.py")]; [compile(p.read_text(), str(p), "exec") for p in paths]; print("python compile ok")'
